    ...
    <section className="page-shell">
      <div className="container">
        <div className="page-head">
          <span className="eyebrow">STALCRAFT Server Panel</span>
        ...
        <div className="control-grid page-control-grid">
          <div className="control-card"><strong>Канал КВ</strong><span>{data.settings?.cw_post_channel_id ? "set" : "—"}</span></div>
          <div className="control-card"><strong>Участвуют</strong><span>{data.attendance.filter((row: any) => row.status === "attending").length}</span></div>
          <div className="control-card"><strong>Табы в очереди</strong><span>{data.resultQueue.length}</span></div>
          <div className="control-card"><strong>Выброс</strong><span>{data.emission?.state === "active" ? "active" : "idle"}</span></div>
        </div>
